...
func SetSwaggerRouter(e *echo.Echo) {

	userSwaggerGroup := e.Group(config.API_V1 + "/swagger/*")
	swaggerHandler := &swaggerAPi{}
	userSwaggerGroup.GET("", swaggerHandler.docs)
}

type swaggerAPi struct{}

func (this *swaggerAPi) docs(ctx *echo.Context) error {

	config := swaggerConfig{
		URLs:                 []string{config.API_V1 + "/swagger/doc.json", config.API_V1 + "/swagger/doc.yaml"},
		DocExpansion:         "list",
		DomID:                "swagger-ui",
	...
	matches := re.FindStringSubmatch(ctx.Request().RequestURI)
	path := matches[2]
	...
	switch path {
	case "":
		_ = ctx.Redirect(http.StatusMovedPermanently, matches[1]+"/"+"index.html")
	case "index.html":
		_ = index.Execute(ctx.Response(), config)
	case "doc.json":
		...
		_, _ = ctx.Response().Write([]byte(doc))
	case "doc.yaml":
		...
		_, _ = ctx.Response().Write(doc)
	default:
		ctx.Request().URL.Path = matches[2]
		http.FileServer(http.FS(swaggerFiles.FS)).ServeHTTP(ctx.Response(), ctx.Request())
	}

	return nil
}

// swaggerConfig stores echoSwagger configuration variables.
